...
async def browser(active_assistant):
    ...
    if active_assistant in app_silero.JARVIS_WAKE_WORDS:
       app_silero.play_audio(jarvis_yes)
...
async def browser_exit(active_assistant):
    ...
    if closed:
        ...
        await app_silero.assistant_tts('Процесс браузера не найден.', 'output.mp3', active_assistant)
...
async def game(active_assistant):
    try:
        ...
        await app_silero.assistant_tts('Путь к файлу не найден, проверьте, правильный ли путь?', 'output.mp3', active_assistant)
        

async def game_exit(active_assistant):
    try:
        if active_assistant in app_silero.JARVIS_WAKE_WORDS:
            app_silero.play_audio(ok_always)
        ...
        await app_silero.assistant_tts('Произошла ошибка при попытке закрытия игры', 'output.mp3', active_assistant)
        

async def offpc(active_assistant):
    '''Отключает ПК под управлением Windows'''
    if active_assistant in app_silero.JARVIS_WAKE_WORDS:
        app_silero.play_audio(jarvis_off)
    os.system('shutdown /s /t 0')
    print("Bot's response: ПК был бы выключен, но команде # в коде мешает ;))) ")
    await app_silero.assistant_tts('ПК был бы выключен, но команде # в коде мешает ;)))', 'output.mp3', active_assistant)
...
async def weather(active_assistant):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            url = f'https://api.openweathermap.org/data/2.5/weather?q={place}&app_sileroid=YOUR_API&units=metric&lang=ru'
            ...
            await app_silero.synthesize_speech(f'Произошла ошибка при попытке запроса к ресурсу API:{e}', 'output.mp3', active_assistant)
        else:
            print(f"Bot's response: На улице {status} {temp} градусов")
            await app_silero.synthesize_speech(f"На улице {status} {temp} градусов", 'output.mp3', active_assistant)
            app_silero.play_audio('output.mp3')
...
async def passive(active_assistant):
    ...
    if active_assistant in app_silero.JARVIS_WAKE_WORDS:
        ...
        await app_silero.assistant_tts(f"Bot's response: Я вас слышаю пассивно.", 'output.mp3', active_assistant)
...
async def time(active_assistant):
    ...
    try:
        await app_silero.assistant_tts(f"Сейчас {current_time}", 'output.mp3', active_assistant)
        
    except:
        print("Bot's response: Произошла ошибка. Вы же можете сам посмотреть на экран компьютера или телефона")
        await app_silero.assistant_tts('Произошла ошибка. Вы же можете сам посмотреть на экран компьютера или телефона', 'output.mp3', active_assistant)
...
async def date(active_assistant):
    ...
    try:
        await app_silero.assistant_tts(f"Сегодня {current_date}", 'output.mp3', active_assistant)
